...
def watch(camera):
    ...
    if (brigthDarkRatio >  THRESHOLD):
        logging.info('start alarm: number of pictures ' + str(NUMBER_OF_PICTURES) + '  every ' + str(SLEEP_TIMELAPSE_SEC) + ' second(s)')
        print('ALARM !!!!')
	takePictures(camera)
    else:
...
if __name__ == "__main__":
    logging.basicConfig(filename='ueberwachung.log',level=logging.INFO)
	...
	GPIO.output(led,GPIO.HIGH)
        ...
        GPIO.output(led,GPIO.LOW)        
        initCamera(camera)
        logging.info('start ueberwachung')
        for i in range(TIME_LAPSE):
            watch(camera)
        logging.info('end ueberwachung')
